    ...
    <div className="flex flex-col">
      ...
      <div className="group relative bg-white rounded-2xl border border-slate-200 shadow-soft hover:shadow-hover hover:-translate-y-0.5 transition-all duration-300 overflow-hidden">
        ...
        <div className="px-6 pb-4 pl-8">
          ...
          {/* Hero row — ring + payment */}
          <div className="flex items-center gap-5 mb-4">
            <ProgressRing percent={m.percentPaid} size={80} stroke={7} color={color} label="paid" />
            <div>
              ...
              <div className="flex items-center gap-2 mt-1.5">
                <span className="text-[11px] font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-full">
                  {mortgage.interestRate}% rate
                </span>
                <span className="text-[11px] text-slate-400">{formatDate(mortgage.startDate)} → {formatDate(m.payoffDate)}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="bg-slate-50 rounded-xl px-3 py-2.5">
              <p className="text-[9px] uppercase tracking-widest text-slate-400 leading-none mb-1">Outstanding</p>
              <p className="text-xs font-bold tabular-nums text-slate-800">{formatCurrency(m.principalRemaining, mortgage.currency)}</p>
            </div>
            <div className="bg-emerald-50 rounded-xl px-3 py-2.5">
              <p className="text-[9px] uppercase tracking-widest text-emerald-500 leading-none mb-1">Principal paid</p>
              <p className="text-xs font-bold tabular-nums text-emerald-700">{formatCurrency(m.principalPaid, mortgage.currency)}</p>
            </div>
            <div className="bg-red-50 rounded-xl px-3 py-2.5">
              <p className="text-[9px] uppercase tracking-widest text-red-400 leading-none mb-1">Interest paid</p>
              <p className="text-xs font-bold tabular-nums text-red-500">{formatCurrency(m.interestPaid, mortgage.currency)}</p>
            </div>
            <div className="bg-slate-50 rounded-xl px-3 py-2.5">
              <p className="text-[9px] uppercase tracking-widest text-slate-400 leading-none mb-1">Payoff date</p>
              <p className="text-xs font-bold text-slate-800">{formatDate(m.payoffDate)}</p>
            </div>
            <div className="bg-red-50/60 rounded-xl px-3 py-2.5 col-span-2">
              <p className="text-[9px] uppercase tracking-widest text-red-400 leading-none mb-1">Interest remaining</p>
              <p className="text-xs font-bold tabular-nums text-red-500">{formatCurrency(m.interestRemaining, mortgage.currency)}</p>
            </div>
          </div>
